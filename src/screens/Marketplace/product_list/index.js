...
export default function ProductsList({ route }) {
  ...
  const onSubmitFilter = (type, value) => {
    console.log('filter');
  ...
  const getSellerArea = () => {
    axiosInstance
      ...
      .catch(error => {
        console.error('getSellerArea', error);
      });
  };
  ...
  const getDeliveryMethod = () => {
    axiosInstance
      ...
      .catch(error => {
        console.error('getDeliveryMethod', error);
      });
  };
  ...
  return (
    <>
      ...
      <View
        ...
        <BodyProduct data={sections} filter={filter} type={route.params.type} />
      </View>
      {/* </Template> */}
      {showFilterButton && (
        <FloatingButton
